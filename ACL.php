...
use Kryn\CmsBundle\Model\Acl as AclObject;
...
class ACL
{
    ...
    function __construct(Core $krynCore, Objects $objects)
    {
    ...
    public function &getRules($objectKey, $mode = 1, $targetType = null, $targetId = null)
    {
        ...
        $user = false;
        if ($targetType === null && $this->getKrynCore()->getClient() && $this->getKrynCore()->getClient()->hasSession()) {
            $user = $this->getKrynCore()->getClient()->getUser();
            $targetType = ACL::USER;
        }

        if ($targetType === ACL::USER && (($user && $targetId && $user->getId() != $targetId) || !$user)) {
            $user = $this->getKrynCore()->getUtils()->getPropelCacheObject('Kryn\\CmsBundle\\Model\\User', $targetId);
        }
        ...
        if ($this->getCaching()) {
            ...
            $cached = $this->getKrynCore()->getDistributedCache('core/acl-rules/' . $cacheKey);
            if (null !== $cached) {
                return $cached;
            }
        }
        ...
        $query = "
                ...
                FROM " . $this->getKrynCore()->getSystemConfig()->getDatabase()->getPrefix(). "system_acl
                WHERE
                object = ? AND
                (mode = ? OR mode = 0) AND
                (
        ...
        if ($this->getCaching()) {
            $this->getKrynCore()->setDistributedCache('core/acl-rules/' . $cacheKey, $rules);
            return $rules;
        } else {
            return $rules;
        }
    ...
    public function getListingCondition($objectKey, $table = '')
    {
        ...
        if ($this->getCaching()) {
            ...
            $cached = $this->getKrynCore()->getDistributedCache('core/acl-listing/' . $cacheKey);
            if (null !== $cached) {
                return $cached;
            }
        }
    ...
    */
    ...
    public function setObject(
        ...
        $query = new \Kryn\CmsBundle\Model\AclQuery();
        $query->select('Prio');
        $query->filterByObject($objectKey);
    ...
    public function check(
        ...
        } elseif (($targetId === null && $targetType === null) && $this->getKrynCore()->getClient() && $this->getKrynCore()->getClient()->hasSession()) {
            $targetId = $this->getKrynCore()->getClient()->getUserId();
            $targetType = ACL::USER;
        }
        ...
        if ($pk && $this->getCaching()) {
            $this->getKrynCore()->setDistributedCache('core/acl/'.$cacheKey, $access);
        }
