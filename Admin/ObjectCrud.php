...
class ObjectCrud extends ContainerAware
{
    ...
    public function translateFields(&$fields)
    {
        if (is_array($fields)) {
            ...
            $fields = $this->getKrynCore()->getTranslator()->t(substr($fields, 2, -2));
        }
    ...
    public function initialize($withoutObjectCheck = false)
    {
        ...
        if ($this->fields) {
            foreach ($this->fields as $key => &$field) {
                if (is_array($field)) {
                    $fieldInstance = new Field(null, $this->getKrynCore());
    ...
    public function translate(&$field)
    {
        if (is_string($field) && substr($field, 0, 2) == '[[' && substr($field, -2) == ']]') {
            $field = $this->getKrynCore()->getTranslator()->t(substr($field, 2, -2));
        }
    }
...
//                        }
    ...
    public function moveItem($sourceUrl, $targetUrl, $position = 'first', $targetObjectKey = '', $overwrite = false)
    {
        ...
        $sourcePk = $this->getKrynCore()->getObjects()->normalizePkString(
            $sourceObjectKey,
            $sourcePkString
        );

        $targetPk = $this->getKrynCore()->getObjects()->normalizePkString(
            $targetObjectKey,
            $targetPkString
        );
     ...
     *
     * @return \Kryn\CmsBundle\Configuration\Condition definition
     */
    public function getCondition()
    {
        return new Condition(null, $this->getKrynCore());
    }
     ...
     *     fixedField2: 'fgh',
     *
     *     _: 'first', //take a look at `$this->getKrynCore()->getObjects()->add()` at parameter `$pPosition`
     *     _pk: {
     *         id: 123132
     *     },
    ...
    public function update($pk, Request $request)
    {
        ...
        $result = $this->getKrynCore()->getObjects()->update(
            $this->getObject(),
            $pk,
            $data,
            array('permissionCheck' => $this->getPermissionCheck())
    ...
    {
        $item['_editable'] = $this->getKrynCore()->getACL()->isUpdatable($this->getObject(), $item);
        $item['_deletable'] = $this->getKrynCore()->getACL()->isDeletable($this->getObject(), $item);
    }
