...
//        }
    ...
    _renderDepends: function (pRes, pDiv) {
        ...
        $H(pRes.depends_ext).each(function (extInfo, extKey) {
            ...
            new Element('img', {
                src: _path + 'bundles/kryncms/admin/images/icons/' + img + '.png',
    ...
    /*
     ...
     new Element('img', {
     src: _path+ PATH_WEB + '/admin/images/ka-tooltip-loading.gif',
     style: 'position: relative; top: 2px; margin-right: 3px;'
     }).inject( div, 'top' );
    ...
    renderInstallForm: function (pValues, pInstall) {
        ...
        if (this.isUpdate) {
            new ka.Button(_('Update now')).addEvent('click', this._install.bind(this, false)).inject(div);
        } else {
    ...
    _renderRemove: function () {
        ...
        new Request.JSON({url: _pathAdmin +
            ...
            if (res.modifiedFiles.length > 0) {
                ...
                new ka.Button(_('Deinstall now')).addEvent('click', this.remove.bind(this)).inject(this.win.content);
