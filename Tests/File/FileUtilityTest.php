...
use Kryn\CmsBundle\Filesystem\FilesystemInterface;
...
class FileUtilityTest extends KernelAwareTestCase
{
    ...
    {
        $this->fileTester($this->getKrynCore()->getCacheFileSystem(), $this->getKernel()->getCacheDir().'/');
    }

    public function testSystemFile()
    {
        $this->fileTester($this->getKrynCore()->getFileSystem(), realpath($this->getKernel()->getRootDir().'/..').'/');
    }

    public function testWebFile()
    {
        $this->fileTester($this->getKrynCore()->getWebFileSystem(), realpath($this->getKernel()->getRootDir().'/../web').'/');
    }
...
";
        ...
        $this->assertInstanceOf('Kryn\\CmsBundle\\File\\FileInfoInterface', $fileObj);
        ...
        $this->assertInstanceOf('Kryn\\CmsBundle\\File\\FileInfoInterface', $files[0]);
        ...
        $this->assertInstanceOf('Kryn\\CmsBundle\\File\\FileInfoInterface', $file1);
